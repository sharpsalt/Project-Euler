...
class Factorial:
    ...
    def fac_inv(self, n: int) -> float:
        """Returns 1 / n! as float (exact if small)"""
        return self.g[n]

    def combi(self, n: int, m: int) -> int:
        """Computes nCm (exact integer)"""
        if m == 0:
            return 1
        if n < m or m < 0 or n < 0:
            return 0
        return self.f[n] // (self.f[m] * self.f[n - m])

    def permu(self, n: int, m: int) -> int:
        """Computes nPm (exact integer)"""
        if n < m or m < 0 or n < 0:
            return 0
        return self.f[n] // self.f[n - m]

    def catalan(self, n: int) -> int:
        """Computes nth Catalan number exactly"""
    ...
    def hockeysticksum(self, n: int, k: int) -> int:
        """Computes Hockey-stick identity: C(n+1, k+1)"""
        return self.combi(n + 1, k + 1)


fact=Factorial(45)
print(int(fact.combi(40,20)))

#if this also is worng i'll try dp
